...
#                "use diamonds reload": True},
           ...
           "375360":
              {"id":"375360",
                "isBuldozer": True,
                "tab":"5",
                "pic":"1697649543042.png",
                #"orderPic":"diamonds",
                #"findWords":"бонус",
                "orderPic":"haltura",
                "findWords":"халтура",
                "use diamonds reload": False,
                "use F5 reload": True},                
#            "betting":
#                {"id":"betting",
#                "tab":"6",
...
#                "btnSubmitABidForProcessing":"btnSubmitABidForProcessing.png",
        ...
        if exists(auto["free"]):
            ...
            if exists(_picFindWords,1): 
                ...
                if exists(auto["btnPlaceABet"]):
                    ...
                    wait(17)
                    if exists(auto["btnSubmitABidForProcessing"]):
                        click()
...
def saveCaptcha():
    ...
    rightClick(getPicZacazZacreplenZaVami().targetOffset(50,30)) 
    try:
...
def checkCaptchaOrder():
    ...
    if auto.get("isBuldozer",False):
        return logger.c(fn + ":: isBuldozer", True)
...
def getIsBuldozer():
    ...
    result = auto.get("isBuldozer",False)
    logger.c(fn + ":: isBuldozer = "+str(result))
    return result
    

#=======================================================================================
def getPicZacazZacreplenZaVami():
    fn = "getPicZacazZacreplenZaVami"
    logger.o(fn)
    
    logger.c(fn)
    return Pattern("1700196221871.png").targetOffset(4,97)
    #if isBuldozer(): 
    #    logger.c(fn)
    #    return Pattern("1700194627621.png").targetOffset(42,-105)
       
    #return Pattern("1700195950865.png").targetOffset(40,-113)
...
def clickOnCaptcha():
    ...
    _captcha2 = getPicZacazZacreplenZaVami()
    captchaFileName = "" 
    if findExists("чтобы взять заказ подтвердите", _captcha2, 1):
        if not checkCaptchaOrder():
            logger.c(fn+":: not checkCaptchaOrder ::return False") 
            return False
...
def getOrderFindWords(i=1):
    ...
    if i==1 and not auto["isBuldozer"]:
...
def getOrderFindWordsPic(i=1):
    ...
    logger.o(fn)
    isBuldozer = getIsBuldozer()
    if isBuldozer:
        logger.c(fn +":: isBuldozer="+ str(isBuldozer)+"; findWords="+auto["findWords"])
        words = auto["findWords"]
        if words == "халтура":
            return Pattern("1698700299155.png").similar(0.90).targetOffset(-51,28)
...
def getOrder():
    ...
    while not isOrderTaken:
        ...
        if status == "empty reload":
            ...
            if scrollToOrder:
                if getIsBuldozer():
                    if findExists("собрать руду", "1698701025979-1.png",0):
                        click()
...
def getAutoStatus():
    ...
    if findExists("взять заказ","vzyati zakaz gray.png",0):
        auto['status'] = "empty reload"
        setTimers()
        logger.c(fn)
        return "empty reload"

    if findExists("reload","1699428337738.png",0):
        auto['status'] = "empty reload"
        setTimers()
        logger.c(fn)
        return "empty reload"
    if findExists("взять заказ","vzyati zakaz blue.png",0):
        auto['status'] = "empty"
        setTimers()
        logger.c(fn)
        return "empty"

    if findExists("order","1700202196628.png",0):
        auto['status'] = "empty"
        setTimers()
        logger.c(fn)
        return "empty"
    ...
    if findExists("заказ принят","_ZacazPrineat.png",0):
        auto['status'] = "order accepted"
        setTimers()
        logger.c(fn)
        return "order accepted"
    if findExists("идет добыча","1699342605033.png",0):
        auto['status'] = "order accepted"
        setTimers()
        logger.c(fn)
        return "order accepted"
    if findExists("сдать заказ","submit order.png",0):
        auto['status'] = "submit order"
        setTimers()
        logger.c(fn)
        return "submit order"

    if findExists("разгрузить","1699353828464.png",0):
        auto['status'] = "submit order"
        setTimers()
        logger.c(fn)
        return "submit order"
    ...
    if findExists("закрыть","1678655733157.png",0):
        auto['status'] = "close"
        setTimers()
        logger.c(fn)
        return "close"

    if findExists("такси отдыхает","otdykhayet.png",0):
        auto['status'] = "resting"
        setTimers()
        logger.c(fn)
        return "resting"

    if findExists("идет разгрузка руды","1698670375322.png",0):
        auto['status'] = "resting"
        setTimers()
        logger.c(fn)
        return "resting"
...
logger.setBaseConfig('log_taxiv3.log')
logger.warning("test")
